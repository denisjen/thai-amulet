...
function LoginForm() {
  ...
  const handleGoogle = async () => {
    ...
    // Google 登入後導向設定密碼頁（已有密碼者會自動跳轉至商品頁）
    await signIn("google", { callbackUrl: "/setup-password" });
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-xl shadow-md p-8">
          <h1 className="text-2xl font-bold text-center mb-6">{isAdminLogin ? "管理員登入" : "會員登入"}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">手機號碼或 Email</label>
              <input
                type="text"
                placeholder="09XXXXXXXX 或 example@mail.com"
                value={form.login}
